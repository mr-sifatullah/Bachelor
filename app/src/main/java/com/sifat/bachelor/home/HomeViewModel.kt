...
class HomeViewModel(private val repository: AppRepository): ViewModel() {
    ...
    fun getUserHomeRentInfo(): LiveData<List<List<String>>> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<List<List<String>>>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.getUserHomeRentInfo(key)
            withContext(Dispatchers.Main) {
                viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
    ...
    fun getUserMealInfo(): LiveData<List<List<String>>> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<List<List<String>>>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.getUserMealInfo(key)
            withContext(Dispatchers.Main) {
                viewState.value = ViewState.ProgressState(false)
                when (response) {
                    is NetworkResponse.Success -> {
